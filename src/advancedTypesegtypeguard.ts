...
//NULLISH COALESCING
//perfectly way to see actually variable have falsy value or has value or has empty string to use =var || 'default' not well here always display this fallback value'default' evenn is empty string
const userInpt='';
//const storedData=userInpt || 'DEFAULT'; //if not coalesc here you get 'default' but if coalesc used here you see empty string that's nice way

const storedData=userInpt ?? 'Default value'; //here its nice becouse taken as empty str not taken as falsy values to return fallback value
