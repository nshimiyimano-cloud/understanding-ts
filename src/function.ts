...
function addno(n1:number,n2:number):number{//if here is number means it return number as result but if here is a string but result is from calculation not concatunation ts throws error that string not assingable as number becouse it calculate as number but it see string
...
function Printresult(num:number):void{ //even no add void ts it addon by default becouse tsis its job
    console.log('result:'+num);
}

Printresult(addno(15,2));
...
combineValues=addno;  //how is done to store function pointer to combineValues
